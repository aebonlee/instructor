...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (!supabase) return
    ;(async () => {
      const { count } = await supabase.from('instructor_users').select('id', { count: 'exact', head: true })
  ...
  const loadMembers = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('id, name, email, provider, created_at, last_login')
        ...
        {tab === 'members' && (
          <>
            ...
            <div className="admin-table-wrapper">
              {memberLoading ? (<div className="admin-empty"><div className="loading-spinner-small" /></div>) : (
                <table className="admin-table">
                  <thead><tr><th>이름</th><th>이메일</th><th>가입수단</th><th>가입일</th><th>최근 로그인</th></tr></thead>
                  <tbody>
                    {pagedMembers.length === 0 ? (
                      ...
                      <tr key={m.id}>
                        ...
                        <td>{fmt(m.created_at)}</td><td>{fmt(m.last_login)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
